	...
	<td class="cart_quantity text-center" data-title="{l s='Qty'}">
		...
		{else}
			...
			{if !isset($customizedDatas.$productId.$productAttributeId) OR $quantityDisplayed > 0}
                ...
                {else}
                        <select class="quantity_wanted_fabric" type="text" name="qty" id="quantity_wanted_fabric_{$product.id_product}_{$product.id_product_attribute}_{if $quantityDisplayed > 0}nocustom{else}0{/if}_{$product.id_address_delivery|intval}">
                        {for $i=1 to $product.quantity_available step 1}
                            <option value="{$i}" {if $product.cart_quantity eq $i}selected="selected"{/if}>{$i*.25} metres</option>
                        {/for}
                        </select>
                {/if}
